...
local function normalize_prompt(out)
    if not out then
        return ""
    end
    -- Remove shell-specific prompt wrappers (zsh/bash) that look garbled in Yazi UI.
    return out
        :gsub("%%{.-%%}", "")
        :gsub("%%{%%}", "")
        :gsub("%%{", "")
...
return {
    ...
    entry = function(_, job)
        ...
        if output_left then
            outputs.left = normalize_prompt(output_left.stdout:gsub("^%s+", ""))
        end
        ...
        if show_right_prompt then
            ...
            if output_right then
                outputs.right = normalize_prompt(output_right.stdout:gsub("^%s+", ""))
            end
        end
