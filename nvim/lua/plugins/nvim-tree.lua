...
M.config = function()
    ...
    vim.api.nvim_set_keymap('n', 'T', '":NvimTreeToggle<cr>"', { silent = true, noremap = true, expr = true })
    vim.keymap.set("n", "E", '":NvimTreeFindFileToggle<cr>"', { silent = true, noremap = true, expr = true })
    vim.api.nvim_command("hi! NvimTreeCursorLine cterm=NONE ctermbg=238")
    vim.api.nvim_command("hi! link NvimTreeFolderIcon NvimTreeFolderName")
    vim.api.nvim_command(
        "au FileType NvimTree nnoremap <buffer> <silent> go :lua require('pack.nvim-tree').magicCd()<cr>")
    M.setup()
end
